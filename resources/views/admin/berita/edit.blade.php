...
@section('content')
    ...
    <div class="row">
        <div class="col-lg">
            <form action="{{ route('data-berita.update', $beritas->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="card">
                    ...
                    <div class="card-body">
                            ...
                            <select name="kategori_id" class="form-control @error('kategori_id') is-invalid @enderror"
                                ...
                                @foreach ($kategoris as $data)
                                    <option value="{{ $data->id }}"
                                        {{ $beritas->kategori_id == $data->id ? 'selected' : '' }}>
                                        {{ $data->nama ?? '-' }}</option>
                                @endforeach
                            </select>
                            @error('kategori_id')
                        ...
                        <div class="mb-3">
                            ...
                            <input type="date" name="tgl_berita"
                                ...
                                value="{{ old('tgl_berita', $beritas->tgl_berita ?? '') }}" placeholder="Masukan ID Status">
                            @error('tgl_berita')
                                <div class="invalid-feedback">
                                    {{ $message }}
                                </div>
                        ...
                        <div class="mb-3">
                            ...
                            <textarea name="judul_berita" class="form-control @error('judul_berita') is-invalid @enderror" rows="5"
                                placeholder="Masukan Judul Berita">{{ old('judul_berita', $beritas->judul_berita ?? '') }}</textarea>
                            @error('judul_berita')
                                <div class="invalid-feedback">
                                    {{ $message }}
                                </div>
                        ...
                        <div class="mb-3">
                            ...
                            <textarea name="isi_berita" class="form-control @error('isi_berita') is-invalid @enderror" rows="5"
                                placeholder="Masukan Isi Berita" id="isiBeritaId">{{ old('isi_berita', $beritas->isi_berita ?? '') }}</textarea>
                            @error('isi_berita')
                                <div class="invalid-feedback">
                                    {{ $message }}
                                </div>
