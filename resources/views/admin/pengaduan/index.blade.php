        ...
        <div class="col-lg">
            <div class="card">
                <div class="card-header">
                    <a href="{{ route('data-pengaduan.create') }}" class="btn btn-primary">
        ...
        $(document).ready(function() {
            ...
            let myTable = $('#myTable').DataTable({
                ...
                initComplete: function(settings, json) {
                    ...
                    var sudahTervalidasiRoute = "{{ route('data-pengaduan.sudahtervalidasi', ':id') }}"
                    var diTolakRoute = "{{ route('data-pengaduan.ditolak', ':id') }}"
                    var editRoute = "{{ route('data-pengaduan.edit', ':id') }}"
                    var showRoute = "{{ route('data-pengaduan.show', ':id') }}"
                    var deleteRoute = "{{ route('data-pengaduan.destroy', ':id') }}";

                    function format(d) {
                        // d is the original data object for the row

                        var sudahTervalidasiUrl = sudahTervalidasiRoute.replace(':id', d.id);
                        var ditolakUrl = diTolakRoute.replace(':id', d.id);
                        var editUrl = editRoute.replace(':id', d.id);
                        var showUrl = showRoute.replace(':id', d.id);
                        var deleteUrl = deleteRoute.replace(':id', d.id);
                        return '<table>' +
                            '<tr>' +
                            '<td>Edit Pengaduan :</td>' +
                            '<td><a href="' + editUrl +
                            '"class="btn btn-info"><i class="fa fa-edit"></i> &nbsp; Edit Pengaduan</a></td>' +
                            '</tr>' +
                            '<tr>' +
                            '<tr>' +
                            '<td>show Pengaduan :</td>' +
                            '<td><a href="' + showUrl +
                            '"class="btn btn-warning"><i class="fa fa-edit"></i> &nbsp; Show Pengaduan</a></td>' +
                            '</tr>' +
                            '<tr>' +
                            '<td>Hapus Pengaduan</td>' +
                            '<td><form action="' + deleteUrl + '" method="POST" id="dataForm">' +
                            '@csrf' +
                            '<button type="submit" class="btn btn-danger" id="hapusData"><i class="fa fa-times"></i> &nbsp; Hapus Pengaduan</button>' +
                            '</form></td>' +
                            '</tr>' +
                            '<tr>' +
                            '<td>Sudah Tervalidasi</td>' +
                            '<td><form action="' + sudahTervalidasiUrl +
                            '" method="POST" id="dataForm">' +
                            '@csrf' +
                            '<button type="submit" class="btn btn-success"><i class="fa fa-check"></i> &nbsp; Sudah Tervalidasi</button>' +
                            '</form></td>' +
                            '</tr>' +
                            '<tr>' +
                            '<td>Ditolak Pengaduan</td>' +
                            '<td><form action="' + ditolakUrl +
                            '" method="POST" id="dataForm">' +
                            '@csrf' +
                            '<button type="submit" class="btn btn-danger"><i class="fa fa-times"></i> &nbsp; Ditolak Pengaduan</button>' +
                            '</form></td>' +
                            '</tr>' +
                            '<tr>' +
